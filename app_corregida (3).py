...
def cargar_todo(url, opcion, cortes):
    try:
        ...
        if opcion == "Resumen Semanal":
            # Para Resumen Semanal, usar extraer_stats_diarias con los cortes correctos
            corte = CORTES.get(opcion, {})
            stats = extraer_stats_diarias(df, corte.get("der_nombres", 5), corte.get("der_cols", (1, 10)))
            
            # Crear DataFrame para mostrar
            df_list = []
            for nombre, stat_dict in stats.items():
                fila = {"Jugador": nombre}
                fila.update(stat_dict)
                df_list.append(fila)
