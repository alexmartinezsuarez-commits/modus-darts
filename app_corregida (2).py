...
def cargar_todo(url, opcion, cortes):
    try:
        if opcion == "Resumen Semanal":
            # Usar cargar_jugadores_desde que ya funciona correctamente
            db = cargar_jugadores_desde(opcion)
            # Crear un DataFrame para mostrar en RESULTADOS
            df_list = []
            for nombre, stats in db.items():
                fila = {"Jugador": nombre}
                fila.update(stats)
                df_list.append(fila)
            df_display = pd.DataFrame(df_list) if df_list else pd.DataFrame()
            return df_display, db
        
        df = pd.read_csv(url, header=None)
        st.session_state.last_update[opcion] = datetime.now()
        
        if opcion == "Value Bets":
